...
% 2.6 Verluste
% ============================================================
\section{Verlustleistung in Dioden}
Eine ideale Diode wirkt im eingeschalteten Zustand wie ein verlustfreier Kurzschluss und kann unbegrenzte Ströme leiten. Im ausgeschalteten Zustand verhält sie sich wie ein Leerlauf und sperrt die anliegende Spannung vollständig. Sie ermöglicht ein unendlich schnelles und vor allem verlustloses Umschalten. 

In der Realität weicht das Verhalten jedoch stark ab. Während der Leitphase fällt eine Vorwärtsspannung ($u_D$) ab, die zu kontinuierlichen Durchlassverlusten führt. Beim Abschalten (Übergang in den Sperrzustand) kann die Diode den Stromfluss nicht sofort stoppen. Durch die im Halbleiter gespeicherte Ladung ($Q$) fließt für kurze Zeit ein negativer Sperrverzögerungsstrom. Da in diesem Moment gleichzeitig die Sperrspannung über der Diode ansteigt, entsteht eine signifikante Verlustleistungsspitze (Schaltverluste). Abbildung \ref{fig:id_diode} verdeutlicht diese parasitären Effekte anhand des Abschaltvorgangs in einem Schaltwandler.

\begin{figure}[htbp]
    \centering
    
    % ==========================================
    % LINKE SEITE: Ideale Diode
    % ==========================================
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                ymin=-4.5, ymax=6,
                xlabel={$t$},
                ylabel={$i_\mathrm{D}, u_\mathrm{D}$},
                % Achsen-Pfeile und Positionierung anpassen
                xtick={0, 4, 8},
                xticklabels={0, $\delta T$, $T$},
                ytick={-3, 2.5, 4.5},
                yticklabels={$-u_\mathrm{i}$, $i_{L,0}$, $i_{L,\delta T}$},
    ...
    \end{minipage}\hfill
    % ==========================================
    % RECHTE SEITE: Reale Diode (mit Verlusten)
    % ==========================================
    \begin{minipage}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
    ...
    \caption{Spannungs- und Stromverlauf im Abwärtswandler. Links: Ideale Diode Rechts: Reale Diode.}
    \label{fig:id_diode}
\end{figure}

\subsection{Durchlassverluste}
Durchlassverluste ergeben sich aus:

\begin{equation}
    P_\mathrm{D,c}=\frac{1}{T}\int_{0}^{T}i_\mathrm{D}u_\mathrm{D}(i_\mathrm{D})\mathrm{d}t
    \label{eq:p_d_con}
\end{equation}

Wir ersetzen Diodenkennlinie durch Serienschaltung aus Spannungsquelle und Widerstand.
