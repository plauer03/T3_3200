    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                xlabel={$t$}, ylabel={$i_\mathrm{S}, u_\mathrm{S}$},
                xtick={0, 4, 8}, xticklabels={0, $\delta T$, $T$},
                ytick={3.5, 1.5, 2.5}, yticklabels={$u_\mathrm{i}$, $i_{L,0}$, $i_{L,\delta T}$},
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \draw (0, 6) to[european voltage source, /tikz/circuitikz/bipoles/length=0.840cm, l_={\textcolor{rgb,255:red,0;green,97;blue,255}{$u_\mathrm{dr}$}}, label distance=0.32cm] (0, 4);
        \draw (0, 4) -- (3, 4);
...
Zur praxisnahen Approximation der Schaltverluste für eine induktive Last wird das vereinfachte lineare Modell der Infineon Application Note \cite{Infineon_MOSFET_Losses} herangezogen. Die Summe aus Einschalt- ($P_{\mathrm{MOS,on}}$) und Ausschaltverlusten ($P_{\mathrm{MOS,off}}$) ergibt sich zu:

\begin{equation}
    P_{\mathrm{MOS,sw}} = P_\mathrm{MOS,on} + P_\mathrm{MOS,off} = \frac{1}{2} u_{\mathrm{i}} \cdot i_{\mathrm{D}} \cdot (t_\mathrm{r} + t_\mathrm{f}) \cdot f_{\mathrm{SW}}
    \label{eq:p_sw}
\end{equation}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                xlabel={$t$}, ylabel={$i_\mathrm{D}, u_\mathrm{D}$},
                xtick={0, 4, 8}, xticklabels={0, $\delta T$, $T$},
                ytick={-3, 2.5, 4.5}, yticklabels={$-u_\mathrm{i}$, $i_{L,0}$, $i_{L,\delta T}$},
                ticklabel style={font=\small},
                xticklabel style={fill=white, fill opacity=0.7, text opacity=1, inner sep=1pt},
                enlargelimits=false, clip=false
            ]
...
Während der Leitphase fällt an der Diode eine nichtlineare Durchlassspannung $u_\mathrm{D}$ ab. Die mittleren Leitverluste berechnen sich über das Integral der Momentanleistung:

\begin{equation}
    P_{\mathrm{D,c}} = \frac{1}{T} \int_{0}^{T} i_\mathrm{D}(t) \cdot u_\mathrm{D}(t) \, \mathrm{d}t
    \label{eq:p_d_con}
\end{equation}

Zur analytischen Berechnung wird die Diodenkennlinie häufig durch eine Reihenschaltung aus einer Schwellenspannung $u_{\mathrm{D,0}}$ und einem differentiellen Widerstand $r_\mathrm{D}$ approximiert. Durch diese Annäherung lassen sich die resultierenden Verluste in einen spannungs- und einen widerstandsabhängigen Anteil ($P_\mathrm{U}$ und $P_\mathrm{R}$) unterteilen. Mithilfe dieser Näherung ergibt sich:

\begin{equation}
    P_{\mathrm{D,c}} = P_\mathrm{U} + P_\mathrm{R} = u_{\mathrm{D,0}} \overline{i}_{\mathrm{D}} + r_\mathrm{D}i_{\mathrm{D,RMS}}^2
...
\begin{equation}
    P_{\mathrm{rr}} = \frac{1}{2}u_{\mathrm{i}} \cdot I_{\mathrm{RM}}\frac{1}{2}t_\mathrm{rr} \cdot f_{\mathrm{SW}}
    \label{eq:p_rr}
\end{equation}
