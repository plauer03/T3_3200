...
\usepackage[scaled=0.85]{beramono} % Lädt die Schrift
\usepackage[T1]{fontenc}
\usepackage[ngerman]{babel}
\usepackage[nopatch=footnote]{microtype} % Typografische Feinheiten
\usepackage{upgreek} % mu, pi etc nicht kurisv
\usepackage{float}
...
\usepackage{rotating} % Für die sidewaystable oder sidewaysfigure Umgebung
\usepackage{afterpage}
% \usepackage{sfmath} % full on arial

% --- CODE LISTINGS
\usepackage{listings}
\usepackage{xcolor} % Wichtig, da wir in \lstset Farben wie "blue" oder "gray" verwenden
...
\usepackage{pgfplots}
\pgfplotsset{compat=1.18} % Wichtig, damit die Graphen korrekt skaliert werden
...
\renewcommand{\mu}{\upmu} % hier neu definiert
...
\newcommand{\hochschule}{Dualen Hochschule Baden-Württemberg Karlsruhe}
